...
def add_contact(args):
    if not args or len(args) < 2 or len(args) > 2:
        print(f"{IDENT}{BOT_ERROR_COLOR}Command's format is wrong. Please use 'help' to check list of commands{Style.RESET_ALL}")
        return
    username = args[0].capitalize()
    phone = args[1]
    users[username] = phone
    print(f"{IDENT}{BOT_COLOR}Contact added.{Style.RESET_ALL}")
...
def update_contact(args):
    ...
    if user_record:
        ...
        print(f"{IDENT}{BOT_ERROR_COLOR}User with  username '{username}' doesn't exist{Style.RESET_ALL}")
        return
    print(f"{IDENT}{BOT_COLOR}Contact updated.{Style.RESET_ALL}")


def get_users_phone(args: list):
    if not args or len(args) > 1:
        print(f"{IDENT}{BOT_ERROR_COLOR}Command is not complete. Please use 'help' to check list of commands{Style.RESET_ALL}")
        return
    if args and args[0]:
        phone = users.get(args[0])
        if not phone:
            print(f"{IDENT}{BOT_COLOR}There is no user wiht name {str(args[0]).capitalize()}{Style.RESET_ALL}")
            return
    print(f"{IDENT}{BOT_COLOR}{args[0]}'s phone is {phone}{Style.RESET_ALL}")
...
def main():
    ...
    while True:
        ...
        if command in ["close", "exit"]:
            ...
            get_users_phone(args)
        elif command == "all":
            print_dict_as_list(users)
        elif command == "help":
            print_dict_as_list(commands)
        else:
            print(f"{IDENT}{BOT_ERROR_COLOR}Invalid command.{Style.RESET_ALL}")
